...
;;; TYPE - for example - verify that rax is closure - type T_CLOSURE
...
%define CAR_OF_PAIR LOWER_DATA
%define CDR_OF_PAIR UPPER_DATA
;;; end of OPERATORS ON PAIRS
...
%endmacro
;;; --- ATOMS CONSTANTS ---
;;; Create literal nil, void, bool, int, float, char 
;;; nil [T_NIL] 1 byte
%define MAKE_NIL db T_NIL 
;;; void [T_VOID] 1 byte
%define MAKE_VOID db T_VOID 
;;; bool true/false (in val) [T_BOOL 1] [T_BOOL 0] 2 bytes (tag & val)
%define MAKE_BOOL(val) MAKE_LITERAL T_BOOL, db val
;;; integer [T_INTEGER 1-8] (tag & 8byte integer number) 1+8 bytes
...
;; --- NEW MAKE LITERAL STRINGGGG ---
%macro MAKE_LITERAL_STRING_V 0-* 
  db T_STRING
  dq %0
	%rep %0
    	db %1
...
;; SHIFT STACK
%macro SHIFT_STACK 1 ; %1 = size of frame (constant)
	 push rax
	 mov rax, PARAM_COUNT
	 add rax, 5
	 mov r9, rax
...
;; SHIFT STACK APPLY
%macro SHIFT_STACK_REG 1 ; %1 = size of frame (register)
	push rax
	mov rcx,	%1			;; for loop
	mov rax, PARAM_COUNT
	add rax, 5
	mov r9, rax
	mov rbx,	1	; for indexing
  %%do_shift:
  	dec rax
	dec rcx
	;;stupid--------------
	...
	shl rsi,	3
	mov rdi,	rbp
	sub rdi,	rsi			;; rdi = rbp -(rbx * WORD_SIZE)
	;;/stupid-------------
	mov r8,	[rdi]
	mov [rbp + WORD_SIZE * rax], r8
	cmp rcx,	0
...
;; PUSHES THE FRAME BY 1
%macro SHIFT_STACK_PUSH 2 ; %1 = lower_bound, %2 = upper_bound

	 mov rax,	%2		; upper address bound
	 sub rax,	%1		; lower address bound
	 shr rax, 	3		; rax = num of blocks to shift
	 inc rax			; to not skip the last block
	 mov rcx,	0		; for indexing

	 %%shift:
	 cmp rcx,	rax									; if done
	 je %%done_shift
	 mov rbx,	[%1 + rcx * WORD_SIZE]				; rbx = value from stack
	 mov [%1 + rcx * WORD_SIZE - WORD_SIZE],	rbx	; lower address by 8 <- rbx
	 inc rcx										; next
	 jmp %%shift
	 %%done_shift:
	 sub rsp,	8
...
;; POPS THE FRAME BY 1 - USED TO FIX FRAME IN APPLY AFTER ALTERING STACK 
%macro SHIFT_STACK_POP 2 ; %1 = lower_bound, %2 = upper_bound
	 ...
	 je %%done_shift
	 neg rcx	;; make rcx negative
	 mov r9,	[%2 + rcx * WORD_SIZE]				; r9 = value from stack
	 mov qword[%2 + rcx * WORD_SIZE],	 0x0
	 mov [%2 + rcx * WORD_SIZE + WORD_SIZE],	r9	; lower address by 8 <- r9
     neg rcx	;; make rcx positive again
	 inc rcx										; next
	 jmp %%shift
	 %%done_shift:
	 add rsp,	8
